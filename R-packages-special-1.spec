...
BuildRequires: R-base >= %{Rversion} gcc-objc gcc gcc-c++
# gcc-objc++ gcc-gfortran
...
%prep
mkdir -p -m 757 %buildroot/usr/lib/R/library/
rm -rf %buildroot/usr/lib/R/library/*
%build
mkdir -p -m 757 %buildroot/usr/lib/R/library/
rm -rf %buildroot/usr/lib/R/library/*
for pkg in %{packages}
...
cat > install.R <<EOF
pkgs <- c($pkgs)
install.packages(pkgs, contriburl='http://vrdc3203/cran/src/contrib', lib='%buildroot/usr/lib/R/library/') 
EOF
R --vanilla < install.R
...
for pkg in %{packages}
do
echo "/usr/lib/R/library/$pkg" >> filelist.R
done
echo "/usr/lib/R/library/R.css" >> filelist.R
